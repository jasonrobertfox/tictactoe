  ...
  module Adapter
    class ThreeSquaredBoardWebAdapter
      ...
      def get_response(request_data)
        ...
        game_state = create_game_state(turn_piece, request_data)
        validate_game_state(game_state)
        new_board = Tictactoe::Player::PerfectPlayer.new(turn_piece).take_turn(game_state)
      ...
      private
        ...
        def create_game_state(turn_piece, request_data)
        ...
        def validate_game_state(game_state)
          fail ArgumentError, 'Nothing to do, the board provided is a draw.' if game_state.draw?
          fail ArgumentError, 'Nothing to do, there is already a winner.' if game_state.winner_exists?
        end
